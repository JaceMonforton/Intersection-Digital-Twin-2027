...
class DittoCarlaBridge:
    def __init__(self):
        ...
        # 1. Connect to CARLA
        logger.info(
            "Connecting to CARLA server at %s:%d...",
            CARLA_HOST,
            CARLA_PORT,
        )
        ...
        logger.info(
            "Spawned Digital Twin Vehicle (Actor ID: %d)",
            self.actor.id,
        )

    def on_open(self, ws):
        logger.info(
            "WebSocket connected to Ditto! Sending event subscription command..."
        )
    ...
    def on_message(self, ws, message):
        ...
        if message.startswith("START-SEND-EVENTS:ACK"):
            logger.info(
                "✅ Ditto Event Subscription Confirmed! "
                "Waiting for twin telemetry..."
            )
            return
        ...
        try:
            ...
            if "/features/kinematics" in path and isinstance(value, dict):
                ...
                # Physical Right-Handed ENU -> CARLA Left-Handed
                ...
                if self.actor is not None:
                    ...
                    logger.info(
                        (
                            "Twin Move -> Phys(x=%.2f, y=%.2f) | "
                            "CARLA(x=%.2f, y=%.2f, yaw=%.1f°)"
                        ),
                        x_phys,
                        y_phys,
                        carla_x,
    ...
    def start(self):
        logger.info(
            "Connecting to Ditto WebSocket at %s...",
            DITTO_WS_URL,
        )
    ...
    def cleanup(self):
        ...
        if self.actor is not None:
            self.actor.destroy()
            logger.info("Cleaned up CARLA vehicle actor.")
